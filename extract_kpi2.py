...
- EXTREMELY IMPORTANT: **Return one object per distinct drug development program or major clinical trial ONLY IF that program or trial is directly relevant to (strictly requiring an explicit mention or a very strong, unambiguous contextual link), or explicitly mentions, the metrics provided in `{search_metric}`.** For each such relevant program/trial, a "distinct" object is warranted for each:
